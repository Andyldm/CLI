...
def con_learn():
    ...
    csv_writer.writerow(["datasets", "f1-ori", "f1-cl", "f1-cli", "auc-ori", "auc-cl", "auc-cli", "acc-ori", "acc-cl",
                         "mcc-cli", "mcc-ori", "mcc-cl", "mcc-cli"])
        ...
        for ix in range(10):
            ...
            for train_index, test_index in sfolder.split(ob, label):
                ...
                for k, (cv_train_idx, cv_holdout_idx) in enumerate(kf.split(X_train, y_train)):
                    ...
                    X_resampled, y_resampled = ros.fit_sample(X_train_cv, s_train_cv)
                ...
                y_original, thresholds1, acc1, mcc1 = get_noise(y_train, psx, X_train, X_test, y_test,seed[ix])
                y_or1.append(y_original)
                auc_or1.append(thresholds1)
                acc_or1.append(acc1)
                mcc_or1.append(mcc1)

                y_original, thresholds2, acc2, mcc2 = get_noise(y_train, psx1, X_train, X_test, y_test,seed[ix])
